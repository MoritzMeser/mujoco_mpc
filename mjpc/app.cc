...
//        physics_pool.Schedule([]() { PhysicsLoop(*sim); });
...

        {
            ...
            cv::VideoWriter video("/Users/moritzmeser/Desktop/video_3.mp4", cv::VideoWriter::fourcc('M', 'J', 'P', 'G'),
                                  10,
                                  cv::Size(3412, 1920));
            for (int i = 0; i < 100; i++) {
                {
                ...
                std::optional<unsigned char *> curr_frame = sim->Render();
                if (curr_frame.has_value()) {

                    unsigned char *ptr = curr_frame.value();
                    std::vector<unsigned char> vec(ptr, ptr + 3412 * 1920 * 3);

                    // Convert the captured frame to a cv::Mat
                    cv::Mat frame(1920, 3412, CV_8UC3, vec.data());
